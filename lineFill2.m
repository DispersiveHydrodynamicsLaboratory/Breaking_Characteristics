function [z,t] = lineFill2(z_shape,t_shape,params,opts,fill)

N = opts.N;
if strcmp(fill,'parallel')
    [z,t]   = f_parallel(z_shape,t_shape,N);
elseif strcmp(fill,'dsw')
    [z,t] = f_dsw(params.zb,params.tb,params.A,N);
elseif strcmp(fill,'rw')
    [z,t] = f_rw(params.zb,params.tb,params.A,N);
else
...
ind = find(diff(A0_vec)~=0,1,'last')+1;
A0 = linspace(Ab,1,N);
t0 = interp1(fliplr(A0_vec(1:ind)),t_vec(1:ind),fliplr(A0)); % contours to be plotted
A0 = fliplr(A0);
z = zeros(N,2);
m = zeros(N,1);
t = zeros(N,2);
